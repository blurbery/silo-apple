...
struct MainTabView: View {
    ...
    /// Visible tabs plus a stable Downloads destination. Capability hydration
    /// controls the contents and download actions, never the tab bar's shape;
    /// this keeps navigation complete on the first authenticated frame.
    private var visibleDestinations: [MainTabDestination] {
        ...
        #if !os(tvOS)
        if !destinations.contains(where: { $0.id == .app(.downloads) }) {
